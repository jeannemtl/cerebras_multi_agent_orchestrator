...
"$PY" -u bridge.py --port $PORT --stripe mock --escrow --budget "$BUDGET" \
    > /tmp/cerebrain_bridge.log 2>&1 &
BRIDGE_PID=$!
echo "      PID $BRIDGE_PID, logs: /tmp/cerebrain_bridge.log"

# Wait for bridge to be ready
sleep 2
if ! lsof -ti:$PORT >/dev/null 2>&1; then
    echo "      ERROR: Bridge failed to start. Check /tmp/cerebrain_bridge.log"
    cat /tmp/cerebrain_bridge.log | tail -20
    exit 1
fi
...
"$PY" -u agent_client.py --all --count "$AGENT_COUNT" --url ws://localhost:$PORT --model "$MODEL" \
    > /tmp/cerebrain_agents.log 2>&1 &
AGENTS_PID=$!
echo "      PID $AGENTS_PID, logs: /tmp/cerebrain_agents.log"
...
echo " Bridge log: /tmp/cerebrain_bridge.log"
echo " Agents log: /tmp/cerebrain_agents.log"
echo ""
